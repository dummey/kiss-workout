import type { TrackerData } from './types'

export const SEED_DATA: TrackerData = {
  "meta": {
    "method": "GZCL",
    "created": "2026-09-01T00:00:00.000Z"
  },
  "exercises": [
    {
      "id": "barbell-back-squat",
      "name": "Barbell Back Squat",
      "muscles": [
        "Quads",
        "Glutes",
        "Hamstrings",
        "Core",
        "Erector Spinae"
      ],
      "setup": "Spotter at 18/2",
      "tier": "T1"
    },
    {
      "id": "barbell-bench",
      "name": "Barbell Bench",
      "muscles": [
        "Chest",
        "Front Delts",
        "Triceps"
      ],
      "setup": "Spotter at 8/-3",
      "tier": "T1"
    },
    {
      "id": "deadlift",
      "name": "Deadlift",
      "muscles": [
        "Hamstrings",
        "Glutes",
        "Erector Spinae",
        "Adductors",
        "Traps"
      ],
      "setup": "",
      "tier": "T1"
    },
    {
      "id": "alt-belt-squat",
      "name": "Alt: Belt Squat",
      "muscles": [
        "Quads",
        "Glutes",
        "Adductors"
      ],
      "setup": "",
      "tier": "T2"
    },
    {
      "id": "db-bench",
      "name": "DB Bench",
      "muscles": [
        "Chest",
        "Front Delts",
        "Triceps"
      ],
      "setup": "DB, Bench",
      "tier": "T2"
    },
    {
      "id": "single-leg-rdl",
      "name": "Single Leg RDL",
      "muscles": [
        "Hamstrings",
        "Glutes",
        "Adductors",
        "Core"
      ],
      "setup": "DB",
      "tier": "T2"
    },
    {
      "id": "bulgarian-split-squats",
      "name": "Bulgarian Split Squats",
      "muscles": [
        "Quads",
        "Glutes",
        "Hamstrings",
        "Adductors"
      ],
      "setup": "Place at -3",
      "tier": "T2"
    },
    {
      "id": "nordics",
      "name": "Nordics",
      "muscles": [
        "Hamstrings",
        "Glutes"
      ],
      "setup": "Lowest 5/8\"",
      "tier": "T2"
    },
    {
      "id": "bent-over-rows",
      "name": "Bent over Rows",
      "muscles": [
        "Lats",
        "Rhomboids",
        "Biceps",
        "Rear Delts",
        "Erector Spinae"
      ],
      "setup": "BB, Spotters",
      "tier": "T2"
    },
    {
      "id": "db-overhead-press",
      "name": "DB Overhead Press",
      "muscles": [
        "Front Delts",
        "Side Delts",
        "Triceps",
        "Upper Traps"
      ],
      "setup": "DB, Bench",
      "tier": "T2"
    },
    {
      "id": "lat-pulldown",
      "name": "Lat Pulldown",
      "muscles": [
        "Lats",
        "Biceps",
        "Rear Delts",
        "Rhomboids"
      ],
      "setup": "Cable",
      "tier": "T3"
    },
    {
      "id": "pallof-press",
      "name": "Pallof Press",
      "muscles": [
        "Obliques",
        "Transversus Abdominis",
        "Rectus Abdominis",
        "Glutes",
        "Scapular Stabilizers",
        "Delts",
        "Triceps"
      ],
      "setup": "Bands",
      "tier": "T3"
    },
    {
      "id": "face-pulls",
      "name": "Face Pulls",
      "muscles": [
        "Rear Delts",
        "Mid/Lower Trap",
        "Rhomboids",
        "Rotator Cuff",
        "Biceps Brachii"
      ],
      "setup": "Cable",
      "tier": "T3"
    },
    {
      "id": "copenhagen-planks",
      "name": "Copenhagen planks",
      "muscles": [
        "Adductors",
        "Core",
        "Obliques"
      ],
      "setup": "Bench",
      "tier": "T3"
    },
    {
      "id": "single-arm-rows",
      "name": "Single Arm Rows",
      "muscles": [
        "Lats",
        "Rhomboids",
        "Biceps",
        "Rear Delts"
      ],
      "setup": "Cable",
      "tier": "T3"
    },
    {
      "id": "glute-kickbacks",
      "name": "Glute Kickbacks (Sideways)",
      "muscles": [
        "Gluteus minimus / medius"
      ],
      "setup": "Cable",
      "tier": "T3"
    },
    {
      "id": "tib-raises",
      "name": "Tib Raises",
      "muscles": [
        "Tibialis anterior"
      ],
      "setup": "Tib Bar, Bench",
      "tier": "T3"
    },
    {
      "id": "calf-raises",
      "name": "Calf Raises (Dumbbells)",
      "muscles": [
        "Gastrocnemius",
        "Soleus"
      ],
      "setup": "DB",
      "tier": "T3"
    },
    {
      "id": "cable-rotations",
      "name": "Cable Rotations",
      "muscles": [
        "Obliques",
        "Transversus Abdominis",
        "Rectus Abdominis"
      ],
      "setup": "Cable",
      "tier": "T3"
    },
    {
      "id": "cable-leg-curls",
      "name": "Cable Leg Curls",
      "muscles": [
        "Hamstrings"
      ],
      "setup": "Cable",
      "tier": "T3"
    },
    {
      "id": "cable-hip-flexion-leg-extension",
      "name": "Cable Hip Flexion + Leg Extension",
      "muscles": [
        "Hip Flexors",
        "Quads"
      ],
      "setup": "Cable",
      "tier": "T3"
    },
    {
      "id": "db-curls",
      "name": "DB Curls",
      "muscles": [
        "Biceps",
        "Brachialis",
        "Forearms"
      ],
      "setup": "DB",
      "tier": "T3"
    },
    {
      "id": "lat-raises",
      "name": "Lat Raises",
      "muscles": [
        "Side Delts"
      ],
      "setup": "Cable",
      "tier": "T3"
    },
    {
      "id": "ab-wheel",
      "name": "Ab Wheel",
      "muscles": [
        "Rectus Abdominis",
        "Obliques",
        "Transversus Abdominis",
        "Hip Flexors"
      ],
      "setup": "",
      "tier": "T3"
    },
    {
      "id": "triceps-pull-down",
      "name": "Triceps Pull Down",
      "muscles": [
        "Triceps"
      ],
      "setup": "Cable",
      "tier": "T3"
    },
    {
      "id": "farmer-carry",
      "name": "Farmer Carry",
      "muscles": [
        "Forearms",
        "Traps",
        "Core",
        "Quads",
        "Grip"
      ],
      "setup": "DB",
      "tier": "T3"
    }
  ],
  "workouts": [
    {
      "name": "Squat Workout",
      "exercises": [
        "barbell-back-squat",
        "alt-belt-squat",
        "db-bench",
        "single-leg-rdl",
        "lat-pulldown",
        "pallof-press",
        "face-pulls",
        "copenhagen-planks",
        "single-arm-rows"
      ]
    },
    {
      "name": "Bench Workout",
      "exercises": [
        "barbell-bench",
        "bulgarian-split-squats",
        "nordics",
        "glute-kickbacks",
        "tib-raises",
        "calf-raises",
        "cable-rotations",
        "cable-leg-curls",
        "cable-hip-flexion-leg-extension"
      ]
    },
    {
      "name": "Deadlift Workout",
      "exercises": [
        "deadlift",
        "bent-over-rows",
        "db-overhead-press",
        "db-curls",
        "lat-raises",
        "ab-wheel",
        "triceps-pull-down",
        "farmer-carry"
      ]
    },
    {
      "name": "Unscheduled",
      "exercises": []
    },
    {
      "name": "Bench Workout (Volume)",
      "exercises": [
        "barbell-bench",
        "bulgarian-split-squats",
        "nordics",
        "glute-kickbacks",
        "tib-raises",
        "calf-raises",
        "cable-rotations",
        "cable-leg-curls",
        "cable-hip-flexion-leg-extension"
      ]
    }
  ],
  "sessions": [
    {
      "date": "2026-10-18",
      "workoutName": "Bench Workout",
      "elapsedTime": 4636,
      "notes": "Superset day: Bench 185\u00d75 then Bench 135\u00d710 for volume.",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "190",
          "reps": "5",
          "sets": 4
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "65",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "10",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "25",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "75",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "45",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "14",
          "sets": 3
        },
        {
          "id": "barbell-bench-copy-001",
          "originalId": "barbell-bench",
          "name": "Barbell Bench (2)",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "135",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-10-17",
      "workoutName": "Squat Workout",
      "elapsedTime": 5080,
      "notes": "Added Face Pulls for shoulder health \u2014 not in regular template.",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "195",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "95",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "55",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "10",
          "reps": "14",
          "sets": 3
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "15",
          "reps": "14",
          "sets": 3
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Rhomboids",
            "External Rotators"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "15",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-10-14",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 2874,
      "notes": "Skipped T3s \u2014 early meeting. Only did main lift and accessories.",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "325",
          "reps": "4",
          "sets": 3
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "150",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "60",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-10-13",
      "workoutName": "Bench Workout",
      "elapsedTime": 4556,
      "notes": "T1 sets: 185\u00d75, 185\u00d73, 185\u00d71. Building volume with same weight.",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "190",
          "reps": "4",
          "sets": 4
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "60",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "25",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "70",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "45",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "13",
          "sets": 3
        },
        {
          "id": "barbell-bench-copy-002",
          "originalId": "barbell-bench",
          "name": "Barbell Bench (2)",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "185",
          "reps": "3",
          "sets": 3
        },
        {
          "id": "barbell-bench-copy-003",
          "originalId": "barbell-bench",
          "name": "Barbell Bench (3)",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "185",
          "reps": "1",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-10-11",
      "workoutName": "Squat Workout",
      "elapsedTime": 4200,
      "notes": "Regular squat day. This session is #10 without backup \u2014 reminder should show.",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "235",
          "reps": "5",
          "sets": 4
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "195",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "95",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "55",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "140",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-10-10",
      "workoutName": "Squat Workout",
      "elapsedTime": 3020,
      "notes": "Week 12 deload \u2014 reduced volume, focusing on recovery.",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "235",
          "reps": "5",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "195",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "90",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "155",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "10",
          "reps": "12",
          "sets": 3
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "15",
          "reps": "13",
          "sets": 3
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "12",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-10-08",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 3467,
      "notes": "Didn't have time to finish all accessories",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "325",
          "reps": "4",
          "sets": 3
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "150",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "60",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "10",
          "reps": "15",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "90",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-10-06",
      "workoutName": "Bench Workout",
      "elapsedTime": 3582,
      "notes": "Week 12 deload \u2014 reduced volume, focusing on recovery.",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "190",
          "reps": "4",
          "sets": 4
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "60",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "10",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "25",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "70",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "15",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "13",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-10-04",
      "workoutName": "Squat Workout",
      "elapsedTime": 3482,
      "notes": "Depth was good. Need to work on bracing.",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "235",
          "reps": "4",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "195",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "90",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "55",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "150",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "10",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "15",
          "reps": "12",
          "sets": 3
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "10",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-10-03",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 3666,
      "notes": "Mixed grip is working. No rotation issues.",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "325",
          "reps": "3",
          "sets": 3
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "150",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "60",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "10",
          "reps": "13",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "85",
          "reps": "15",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-10-02",
      "workoutName": "Bench Workout",
      "elapsedTime": 2702,
      "notes": "Ran out of time on accessories",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "190",
          "reps": "4",
          "sets": 4
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "10",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "70",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "14",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        }
      ]
    },
    {
      "date": "2026-09-30",
      "workoutName": "Squat Workout",
      "elapsedTime": 5008,
      "notes": "Didn't have time to finish all accessories.",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "235",
          "reps": "4",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "195",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "90",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "150",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "15",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-09-27",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 4965,
      "notes": "Only did main lift \u2014 no accessories today.",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "325",
          "reps": "3",
          "sets": 3
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "150",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "60",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "85",
          "reps": "13",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-09-26",
      "workoutName": "Bench Workout",
      "elapsedTime": 2891,
      "notes": "Skipped some T3s \u2014 ran out of time.",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "190",
          "reps": "3",
          "sets": 4
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "60",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "5",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "25",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "70",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "13",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "11",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-09-23",
      "workoutName": "Squat Workout",
      "elapsedTime": 4996,
      "notes": "Depth was good. Need to work on bracing.",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "235",
          "reps": "4",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "150",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "5",
          "reps": "14",
          "sets": 3
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "50",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "15",
          "reps": "11",
          "sets": 3
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "14",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-09-21",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 3746,
      "notes": "Skipped some T3s \u2014 ran out of time",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "325",
          "reps": "3",
          "sets": 3
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "150",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "60",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "25",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "10",
          "reps": "11",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        }
      ]
    },
    {
      "date": "2026-09-20",
      "workoutName": "Bench Workout",
      "elapsedTime": 3662,
      "notes": "Good session. Triceps are catching up.",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "190",
          "reps": "3",
          "sets": 4
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "55",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "5",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "13",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "11",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-09-19",
      "workoutName": "Squat Workout",
      "elapsedTime": 3248,
      "notes": "Didn't have time to finish all accessories.",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "235",
          "reps": "3",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "195",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "90",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "150",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "5",
          "reps": "13",
          "sets": 3
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "50",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "15",
          "reps": "11",
          "sets": 3
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        }
      ]
    },
    {
      "date": "2026-09-18",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 4719,
      "notes": "Didn't have time to finish all accessories",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "315",
          "reps": "6",
          "sets": 4
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "145",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "60",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "10",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "85",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-09-16",
      "workoutName": "Bench Workout",
      "elapsedTime": 2708,
      "notes": "Cut session short \u2014 gym was closing",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "190",
          "reps": "3",
          "sets": 4
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "5",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "70",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "12",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-09-14",
      "workoutName": "Squat Workout",
      "elapsedTime": 2710,
      "notes": "Easy session. Weight moved fast.",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "235",
          "reps": "3",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "195",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "90",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "5",
          "reps": "12",
          "sets": 3
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "50",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "11",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-09-11",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 3346,
      "notes": "Skipped T3s \u2014 early meeting",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "315",
          "reps": "6",
          "sets": 4
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "145",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "60",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "25",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "5",
          "reps": "15",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "85",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-09-08",
      "workoutName": "Bench Workout",
      "elapsedTime": 4737,
      "notes": "Skipped T3s \u2014 early meeting",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "185",
          "reps": "6",
          "sets": 5
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "55",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "5",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "11",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        }
      ]
    },
    {
      "date": "2026-09-06",
      "workoutName": "Squat Workout",
      "elapsedTime": 3817,
      "notes": "Skipped some T3s \u2014 ran out of time",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "235",
          "reps": "3",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "195",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "85",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "145",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "10",
          "reps": "15",
          "sets": 3
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "11",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-09-03",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 2881,
      "notes": "Didn't have time to finish all accessories",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "315",
          "reps": "6",
          "sets": 4
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "145",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "60",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "25",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "5",
          "reps": "14",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "85",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-09-02",
      "workoutName": "Bench Workout",
      "elapsedTime": 3251,
      "notes": "Upper back was fatigued. Stopped early.",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "185",
          "reps": "6",
          "sets": 5
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "55",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "5",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "13",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-09-01",
      "workoutName": "Squat Workout",
      "elapsedTime": 4723,
      "notes": "Skipped T3s \u2014 early meeting",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "225",
          "reps": "6",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "195",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "85",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "10",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-08-29",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 4161,
      "notes": "Felt off. Stopped after T2.",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "315",
          "reps": "5",
          "sets": 4
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "145",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "60",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "5",
          "reps": "13",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "85",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-08-26",
      "workoutName": "Bench Workout",
      "elapsedTime": 4375,
      "notes": "Skipped T3s \u2014 early meeting",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "185",
          "reps": "5",
          "sets": 5
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "55",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "BW",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        }
      ]
    },
    {
      "date": "2026-08-24",
      "workoutName": "Squat Workout",
      "elapsedTime": 5308,
      "notes": "Didn't have time to finish all accessories",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "225",
          "reps": "6",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "195",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "85",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "145",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "50",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "10",
          "reps": "14",
          "sets": 3
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "15",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-08-23",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 4668,
      "notes": "Skipped T3s \u2014 early meeting",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "315",
          "reps": "5",
          "sets": 4
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "145",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "5",
          "reps": "12",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "80",
          "reps": "15",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-08-21",
      "workoutName": "Bench Workout",
      "elapsedTime": 3767,
      "notes": "Sticking point at chest. Need more paused work.",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "185",
          "reps": "5",
          "sets": 5
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "55",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "BW",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "14",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "11",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-08-18",
      "workoutName": "Squat Workout",
      "elapsedTime": 3136,
      "notes": "New PR on top set! Felt explosive.",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "225",
          "reps": "6",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "195",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "85",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "145",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "BW",
          "reps": "15",
          "sets": 3
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "45",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "10",
          "reps": "12",
          "sets": 3
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "13",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-08-16",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 4132,
      "notes": "Cut session short \u2014 gym was closing",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "315",
          "reps": "5",
          "sets": 4
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "145",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "5",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "80",
          "reps": "14",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-08-13",
      "workoutName": "Bench Workout",
      "elapsedTime": 4812,
      "notes": "Skipped T3s \u2014 early meeting",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "BW",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "12",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-08-12",
      "workoutName": "Squat Workout",
      "elapsedTime": 4938,
      "notes": "Cut session short \u2014 gym was closing",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "225",
          "reps": "5",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "190",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "80",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "145",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "BW",
          "reps": "14",
          "sets": 3
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "45",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "11",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-08-09",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 5129,
      "notes": "Grip is becoming an issue. Need straps soon.",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "315",
          "reps": "4",
          "sets": 4
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "140",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "55",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "BW",
          "reps": "15",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        }
      ]
    },
    {
      "date": "2026-08-08",
      "workoutName": "Bench Workout",
      "elapsedTime": 4057,
      "notes": "Great leg drive. Everything felt tight.",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "185",
          "reps": "4",
          "sets": 5
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "BW",
          "reps": "7",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "25",
          "reps": "15",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-08-06",
      "workoutName": "Squat Workout",
      "elapsedTime": 5090,
      "notes": "Skipped some T3s \u2014 ran out of time",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "225",
          "reps": "5",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "80",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "145",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "BW",
          "reps": "13",
          "sets": 3
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "45",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        }
      ]
    },
    {
      "date": "2026-08-04",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 5306,
      "notes": "Didn't have time to finish all accessories",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "315",
          "reps": "4",
          "sets": 4
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "140",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "55",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "BW",
          "reps": "13",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        }
      ]
    },
    {
      "date": "2026-08-03",
      "workoutName": "Bench Workout",
      "elapsedTime": 4971,
      "notes": "Only did main lift \u2014 no accessories today",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "185",
          "reps": "4",
          "sets": 5
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "35",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "50",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        }
      ]
    },
    {
      "date": "2026-08-01",
      "workoutName": "Squat Workout",
      "elapsedTime": 5181,
      "notes": "Didn't have time to finish all accessories.",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "225",
          "reps": "4",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "190",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "80",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "45",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "140",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "BW",
          "reps": "13",
          "sets": 3
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "45",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "5",
          "reps": "14",
          "sets": 3
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "50",
          "reps": "15",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-07-30",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 5022,
      "notes": "Cut session short \u2014 gym was closing",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "315",
          "reps": "3",
          "sets": 4
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "140",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "55",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "BW",
          "reps": "13",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "80",
          "reps": "12",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-07-28",
      "workoutName": "Bench Workout",
      "elapsedTime": 3540,
      "notes": "Felt weak today. Sleep was poor yesterday.",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "185",
          "reps": "4",
          "sets": 5
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "BW",
          "reps": "7",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "15",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "50",
          "reps": "14",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "25",
          "reps": "11",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-07-26",
      "workoutName": "Squat Workout",
      "elapsedTime": 4646,
      "notes": "Knees felt tight. Added extra warmup.",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "225",
          "reps": "4",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "190",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "75",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "45",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "140",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "45",
          "reps": "11",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "5",
          "reps": "12",
          "sets": 3
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        }
      ]
    },
    {
      "date": "2026-07-23",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 3329,
      "notes": "Skipped some T3s \u2014 ran out of time",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "315",
          "reps": "3",
          "sets": 4
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "140",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "BW",
          "reps": "12",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "55",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "80",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-07-21",
      "workoutName": "Bench Workout",
      "elapsedTime": 3567,
      "notes": "Bench is progressing. Lockout is solid.",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "185",
          "reps": "3",
          "sets": 5
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "BW",
          "reps": "6",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "20",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "65",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "14",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "50",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "25",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-07-18",
      "workoutName": "Squat Workout",
      "elapsedTime": 3623,
      "notes": "Only did main lift \u2014 no accessories today",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "225",
          "reps": "3",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "190",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "75",
          "reps": "9",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "140",
          "reps": "13",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "BW",
          "reps": "11",
          "sets": 3
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "45",
          "reps": "10",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "50",
          "reps": "12",
          "sets": 4
        }
      ]
    },
    {
      "date": "2026-07-17",
      "workoutName": "Deadlift Workout",
      "elapsedTime": 4370,
      "notes": "Felt tired, cut accessories early",
      "exercises": [
        {
          "id": "deadlift",
          "name": "Deadlift",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Erector Spinae",
            "Adductors",
            "Traps"
          ],
          "setup": "",
          "tier": "T1",
          "superset": "",
          "weight": "315",
          "reps": "3",
          "sets": 4
        },
        {
          "id": "bent-over-rows",
          "name": "Bent over Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts",
            "Erector Spinae"
          ],
          "setup": "BB, Spotters",
          "tier": "T2",
          "superset": "",
          "weight": "135",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "db-overhead-press",
          "name": "DB Overhead Press",
          "muscles": [
            "Front Delts",
            "Side Delts",
            "Triceps",
            "Upper Traps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "db-curls",
          "name": "DB Curls",
          "muscles": [
            "Biceps",
            "Brachialis",
            "Forearms"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "25",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "lat-raises",
          "name": "Lat Raises",
          "muscles": [
            "Side Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "15",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "ab-wheel",
          "name": "Ab Wheel",
          "muscles": [
            "Rectus Abdominis",
            "Obliques",
            "Transversus Abdominis",
            "Hip Flexors"
          ],
          "setup": "",
          "tier": "T3",
          "superset": "",
          "weight": "BW",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "triceps-pull-down",
          "name": "Triceps Pull Down",
          "muscles": [
            "Triceps"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "farmer-carry",
          "name": "Farmer Carry",
          "muscles": [
            "Forearms",
            "Traps",
            "Core",
            "Quads",
            "Grip"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        }
      ]
    },
    {
      "date": "2026-07-15",
      "workoutName": "Bench Workout",
      "elapsedTime": 3706,
      "notes": "Cut session short \u2014 gym was closing",
      "exercises": [
        {
          "id": "barbell-bench",
          "name": "Barbell Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "Spotter at 8/-3",
          "tier": "T1",
          "superset": "",
          "weight": "185",
          "reps": "3",
          "sets": 5
        },
        {
          "id": "bulgarian-split-squats",
          "name": "Bulgarian Split Squats",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Adductors"
          ],
          "setup": "Place at -3",
          "tier": "T2",
          "superset": "",
          "weight": "50",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "nordics",
          "name": "Nordics",
          "muscles": [
            "Hamstrings",
            "Glutes"
          ],
          "setup": "Lowest 5/8\"",
          "tier": "T2",
          "superset": "",
          "weight": "BW",
          "reps": "6",
          "sets": 3
        },
        {
          "id": "glute-kickbacks",
          "name": "Glute Kickbacks (Sideways)",
          "muscles": [
            "Gluteus minimus / medius"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "15",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "tib-raises",
          "name": "Tib Raises",
          "muscles": [
            "Tibialis anterior"
          ],
          "setup": "Tib Bar, Bench",
          "tier": "T3",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "calf-raises",
          "name": "Calf Raises (Dumbbells)",
          "muscles": [
            "Gastrocnemius",
            "Soleus"
          ],
          "setup": "DB",
          "tier": "T3",
          "superset": "",
          "weight": "60",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "cable-rotations",
          "name": "Cable Rotations",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "30",
          "reps": "12",
          "sets": 3
        },
        {
          "id": "cable-leg-curls",
          "name": "Cable Leg Curls",
          "muscles": [
            "Hamstrings"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "50",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "cable-hip-flexion-leg-extension",
          "name": "Cable Hip Flexion + Leg Extension",
          "muscles": [
            "Hip Flexors",
            "Quads"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "25",
          "reps": "10",
          "sets": 3
        }
      ]
    },
    {
      "date": "2026-07-13",
      "workoutName": "Squat Workout",
      "elapsedTime": 5393,
      "notes": "Ran out of time on accessories",
      "exercises": [
        {
          "id": "barbell-back-squat",
          "name": "Barbell Back Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Hamstrings",
            "Core",
            "Erector Spinae"
          ],
          "setup": "Spotter at 18/2",
          "tier": "T1",
          "superset": "",
          "weight": "225",
          "reps": "3",
          "sets": 3
        },
        {
          "id": "alt-belt-squat",
          "name": "Alt: Belt Squat",
          "muscles": [
            "Quads",
            "Glutes",
            "Adductors"
          ],
          "setup": "",
          "tier": "T2",
          "superset": "",
          "weight": "185",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "db-bench",
          "name": "DB Bench",
          "muscles": [
            "Chest",
            "Front Delts",
            "Triceps"
          ],
          "setup": "DB, Bench",
          "tier": "T2",
          "superset": "",
          "weight": "75",
          "reps": "8",
          "sets": 3
        },
        {
          "id": "single-leg-rdl",
          "name": "Single Leg RDL",
          "muscles": [
            "Hamstrings",
            "Glutes",
            "Adductors",
            "Core"
          ],
          "setup": "DB",
          "tier": "T2",
          "superset": "",
          "weight": "",
          "reps": "",
          "sets": null
        },
        {
          "id": "lat-pulldown",
          "name": "Lat Pulldown",
          "muscles": [
            "Lats",
            "Biceps",
            "Rear Delts",
            "Rhomboids"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "140",
          "reps": "12",
          "sets": 4
        },
        {
          "id": "pallof-press",
          "name": "Pallof Press",
          "muscles": [
            "Obliques",
            "Transversus Abdominis",
            "Rectus Abdominis",
            "Glutes",
            "Scapular Stabilizers",
            "Delts",
            "Triceps"
          ],
          "setup": "Bands",
          "tier": "T3",
          "superset": "",
          "weight": "BW",
          "reps": "10",
          "sets": 3
        },
        {
          "id": "face-pulls",
          "name": "Face Pulls",
          "muscles": [
            "Rear Delts",
            "Mid/Lower Trap",
            "Rhomboids",
            "Rotator Cuff",
            "Biceps Brachii"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "40",
          "reps": "15",
          "sets": 4
        },
        {
          "id": "copenhagen-planks",
          "name": "Copenhagen planks",
          "muscles": [
            "Adductors",
            "Core",
            "Obliques"
          ],
          "setup": "Bench",
          "tier": "T3",
          "superset": "",
          "weight": "BW",
          "reps": "30",
          "sets": 3
        },
        {
          "id": "single-arm-rows",
          "name": "Single Arm Rows",
          "muscles": [
            "Lats",
            "Rhomboids",
            "Biceps",
            "Rear Delts"
          ],
          "setup": "Cable",
          "tier": "T3",
          "superset": "",
          "weight": "50",
          "reps": "12",
          "sets": 4
        }
      ]
    }
  ]
}
